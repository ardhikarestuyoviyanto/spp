    ...
    <tbody>
        <?php $total = 0; $i=1; foreach ($kelas as $x): ?>
            <tr>
                ...
                <td><?php echo "Rp. ".number_format($modelss->getRekapBulanan($x->id_kelas, $_GET['pembayaran'], urldecode($_GET['tahun']), $_GET['start'], $_GET['finish']) ,2,',','.'); ?></td>
            </tr>
            <?php $total = $total + $modelss->getRekapBulanan($x->id_kelas, $_GET['pembayaran'], urldecode($_GET['tahun']), $_GET['start'], $_GET['finish']); ?>
        <?php endforeach; ?>
        <tr class="text-bold">
            <td></td>
            <td>Jumlah</td>
