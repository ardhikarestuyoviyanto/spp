...
<table border="1" class="table table-striped table-bordered">
    ...
    <tbody>
        <?php $total = 0; $i=1; foreach ($kelas as $x): ?>
            <tr>
                <td><?php echo $i++; ?></td>
                <td><?php echo $modelss->LastTransaksiTagihanBulanan($x->nis, $_GET['pembayaran']); ?></td>
                <td><?php echo $x->nama_siswa; ?></td>
                <td><?php echo "Rp. ".number_format($modelss->TotalBayarTagihanBulanan($x->nis, $_GET['pembayaran']) ,2,',','.'); ?></td>
            </tr>
            <?php $total = $total + $modelss->TotalBayarTagihanBulanan($x->nis, $_GET['pembayaran']) ; ?>
        <?php endforeach; ?>
        <tr class="text-bold">
            <td></td>
